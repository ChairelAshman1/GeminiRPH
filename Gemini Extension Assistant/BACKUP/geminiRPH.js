...
function attachListeners() {
    ...
    $('btnSaveKey').addEventListener('click', () => {
        ...
        // Auto switch to API mode if user just saved a key
        if(val.length > 5) {
            CURRENT_MODE = 'API';
        ...
        updateStatusUI();
        showToast("API Key Disimpan. Mode dikemaskini.", "success");
    ...
    // Button: Web Scrape (Only for Web Mode)
    $('btnWebScrape').addEventListener('click', async () => {
        showLoading(true, "Mengambil data...");
        chrome.tabs.query({active:true, currentWindow:true}, tabs => {
            if(!tabs[0]) return;
            chrome.tabs.sendMessage(tabs[0].id, { action: "GET_GEMINI_RESPONSE" }, response => {
                showLoading(false);
                if (response && response.text) {
                    renderOutputs(response.text);
                    showToast("Data berjaya diambil!", "success");
                } else {
                    showToast("Tiada data dijumpai atau ralat.", "error");
                }
            });
        });
    });
...
async function smartGenerate(promptText, actionType, fallbackData = null) {
    if (CURRENT_MODE === 'API') {
        ...
        try {
            ...
            showToast("Had API Dicapai. Menukar ke Web Gemini...", "error");
            
            CURRENT_MODE = 'WEB';
            updateStatusUI();
            
            // Auto trigger web fallback without waiting
...
function renderOutputs(text) {
    ...
    if (objText) {
         // 1. Remove TS25 specific labels in parentheses (e.g. (Audience), (A), etc.)
         objText = objText.replace(/\s*\((?:Audience|Behavior|Behaviour|Condition|Degree|Audiens|Tingkah\s*laku|Syarat|Tahap|[ABCD])\)/gi, "");
         
         // 2. Remove ONLY horizontal double spaces to keep paragraphs clean, but PRESERVE newlines
         objText = objText.replace(/[ \t]{2,}/g, " ");

         // 3. Ensure numbered lists start on new lines if Gemini merged them (e.g. "1. Obj 2. Obj")
         objText = objText.replace(/(\s+)(\d+[\.\)])/g, "\n$2");

         // 4. Split and enforce Limit 6
    ...
    if (kritText) {
         ...
         kritText = kritText.replace(/(\s+)(\d+[\.\)])/g, "\n$2");
